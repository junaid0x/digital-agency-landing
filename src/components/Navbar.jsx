...
const Navbar = ({ theme, setTheme }) => {
    ...
    lg:px-24 xl:px-40 py-4 sticky top-0 z-20 backdrop-blur-xl font-medium
    bg-white/50 dark:bg-gray-900/70'>
        ...
        ${!sidebarOpen ? "max-sm:w-0 max-sm:overflow-hidden" :"max-sm:w-60 max-sm:pl-10"} max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full
        max-sm:flex-col max-sm:bg-primary max-sm:text-white max-sm:pt-20 flex sm:items-center gap-5
        transition-all `}>

                <img onClick={()=>setSidebarOpen(false)} src={assets.close_icon} alt=""
            ...
            <div className='flex items-center gap-2 sm:gap-4'>
                ...
                <img onClick={()=>setSidebarOpen(true)} 
                src={theme === 'dark' ? assets.menu_icon_dark : assets.menu_icon} alt="" 
                className='w-8 sm:hidden' />

                <a href="#contact-us" className='text-sm max-sm:hidden flex 
                items-center gap-2 bg-primary text-white
